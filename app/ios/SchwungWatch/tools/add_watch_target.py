#!/usr/bin/env python3
"""Add the SchwungWatch watchOS target to ios/Runner.xcodeproj (WP-13).

Run once, from the repo's `app/` directory:

    python3 ios/SchwungWatch/tools/add_watch_target.py
    xcodebuild -list -project ios/Runner.xcodeproj      # SchwungWatch must appear
...
SWIFT = ['SchwungWatchApp.swift', 'ContentView.swift', 'WatchModel.swift', 'WatchSessionManager.swift',
...
def main():
    s = open(PATH).read()
    if 'SchwungWatch' in s:
        print('SchwungWatch target already present — nothing to do')
        return 0

    fileref = {n: i('%02X' % (0x10 + k)) for k, n in enumerate(SWIFT)}
    fileref['Info.plist'] = i('18')
    fileref['SchwungWatch.entitlements'] = i('19')
    ...
    add += '\t\t%s /* SchwungWatch.app in Embed Watch Content */ = {isa = PBXBuildFile; fileRef = %s /* SchwungWatch.app */; settings = {ATTRIBUTES = (RemoveHeadersOnCopy, ); }; };\n' % (i('0E'), i('02'))
...
\t\t\tdstSubfolderSpec = 16;
\t\t\tfiles = (
\t\t\t\t%s /* SchwungWatch.app in Embed Watch Content */,
\t\t\t);
...
''' % (i('0D'), i('0E')), 1)
    ...
    add = '\t\t%s /* SchwungWatch.app */ = {isa = PBXFileReference; explicitFileType = wrapper.application; includeInIndex = 0; path = SchwungWatch.app; sourceTree = BUILT_PRODUCTS_DIR; };\n' % i('02')
    for n in SWIFT:
    ...
    add += '\t\t%s /* SchwungWatch.entitlements */ = {isa = PBXFileReference; lastKnownFileType = text.plist.entitlements; path = SchwungWatch.entitlements; sourceTree = "<group>"; };\n' % fileref['SchwungWatch.entitlements']
...
/* End PBXFrameworksBuildPhase section */''' % i('09'), 1)
    ...
    children += '\t\t\t\t%s /* SchwungWatch.entitlements */,\n' % fileref['SchwungWatch.entitlements']
    s = s.replace('/* End PBXGroup section */', '''\t\t%s /* SchwungWatch */ = {
\t\t\tisa = PBXGroup;
\t\t\tchildren = (
%s\t\t\t);
\t\t\tpath = SchwungWatch;
...
/* End PBXGroup section */''' % (i('01'), children), 1)
    ...
    s = s.replace('/* End PBXNativeTarget section */', '''\t\t%s /* SchwungWatch */ = {
\t\t\tisa = PBXNativeTarget;
\t\t\tbuildConfigurationList = %s /* Build configuration list for PBXNativeTarget "SchwungWatch" */;
\t\t\tbuildPhases = (
\t\t\t\t%s /* Sources */,
\t\t\t\t%s /* Frameworks */,
\t\t\t\t%s /* Resources */,
...
\t\t\t);
\t\t\tname = SchwungWatch;
\t\t\tproductName = SchwungWatch;
\t\t\tproductReference = %s /* SchwungWatch.app */;
...
\t\t\t\tCODE_SIGN_ENTITLEMENTS = SchwungWatch/SchwungWatch.entitlements;
\t\t\t\tCODE_SIGN_STYLE = Automatic;
\t\t\t\tCURRENT_PROJECT_VERSION = "$(FLUTTER_BUILD_NUMBER)";
\t\t\t\tDEVELOPMENT_TEAM = 5GDU97KSQU;
\t\t\t\tENABLE_PREVIEWS = YES;
\t\t\t\tGENERATE_INFOPLIST_FILE = NO;
\t\t\t\tINFOPLIST_FILE = SchwungWatch/Info.plist;
\t\t\t\tLD_RUNPATH_SEARCH_PATHS = (
\t\t\t\t\t"$(inherited)",
\t\t\t\t\t"@executable_path/Frameworks",
\t\t\t\t);
\t\t\t\tMARKETING_VERSION = "$(FLUTTER_BUILD_NAME)";
\t\t\t\tPRODUCT_BUNDLE_IDENTIFIER = de.torchtechnology.schwung.watchkitapp;
...
''' % (cid, name, settings, extra, name)
    ...
    s = s.replace('/* End XCConfigurationList section */', '''\t\t%s /* Build configuration list for PBXNativeTarget "SchwungWatch" */ = {
...
/* End XCConfigurationList section */''' % (i('04'), i('05'), i('07'), i('06')), 1)
    ...
    open(PATH, 'w').write(s)
    print('SchwungWatch target added to', PATH)
    return 0
