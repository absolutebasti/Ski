#!/usr/bin/env python3
"""Add the SlopeTrackWatch watchOS target to ios/Runner.xcodeproj (WP-13).

Run once, from the repo's `app/` directory:

    python3 ios/SlopeTrackWatch/tools/add_watch_target.py
    xcodebuild -list -project ios/Runner.xcodeproj      # SlopeTrackWatch must appear
...
SWIFT = ['SlopeTrackWatchApp.swift', 'ContentView.swift', 'WatchModel.swift', 'WatchSessionManager.swift',
...
def main():
    s = open(PATH).read()
    if 'SlopeTrackWatch' in s:
        print('SlopeTrackWatch target already present — nothing to do')
        return 0

    fileref = {n: i('%02X' % (0x10 + k)) for k, n in enumerate(SWIFT)}
    fileref['Info.plist'] = i('18')
    fileref['SlopeTrackWatch.entitlements'] = i('19')
    ...
    add += '\t\t%s /* SlopeTrackWatch.app in Embed Watch Content */ = {isa = PBXBuildFile; fileRef = %s /* SlopeTrackWatch.app */; settings = {ATTRIBUTES = (RemoveHeadersOnCopy, ); }; };\n' % (i('0E'), i('02'))
...
\t\t\tdstSubfolderSpec = 16;
\t\t\tfiles = (
\t\t\t\t%s /* SlopeTrackWatch.app in Embed Watch Content */,
\t\t\t);
...
''' % (i('0D'), i('0E')), 1)
    ...
    add = '\t\t%s /* SlopeTrackWatch.app */ = {isa = PBXFileReference; explicitFileType = wrapper.application; includeInIndex = 0; path = SlopeTrackWatch.app; sourceTree = BUILT_PRODUCTS_DIR; };\n' % i('02')
    for n in SWIFT:
    ...
    add += '\t\t%s /* SlopeTrackWatch.entitlements */ = {isa = PBXFileReference; lastKnownFileType = text.plist.entitlements; path = SlopeTrackWatch.entitlements; sourceTree = "<group>"; };\n' % fileref['SlopeTrackWatch.entitlements']
...
/* End PBXFrameworksBuildPhase section */''' % i('09'), 1)
    ...
    children += '\t\t\t\t%s /* SlopeTrackWatch.entitlements */,\n' % fileref['SlopeTrackWatch.entitlements']
    s = s.replace('/* End PBXGroup section */', '''\t\t%s /* SlopeTrackWatch */ = {
\t\t\tisa = PBXGroup;
\t\t\tchildren = (
%s\t\t\t);
\t\t\tpath = SlopeTrackWatch;
\t\t\tsourceTree = "<group>";
\t\t};
/* End PBXGroup section */''' % (i('01'), children), 1)
    ...
    s = s.replace('\t\t\t\t331C8081294A63A400263BE5 /* RunnerTests.xctest */,\n',
                  '\t\t\t\t331C8081294A63A400263BE5 /* RunnerTests.xctest */,\n\t\t\t\t%s /* SlopeTrackWatch.app */,\n' % i('02'), 1)

    s = s.replace('/* End PBXNativeTarget section */', '''\t\t%s /* SlopeTrackWatch */ = {
\t\t\tisa = PBXNativeTarget;
\t\t\tbuildConfigurationList = %s /* Build configuration list for PBXNativeTarget "SlopeTrackWatch" */;
\t\t\tbuildPhases = (
\t\t\t\t%s /* Sources */,
\t\t\t\t%s /* Frameworks */,
\t\t\t\t%s /* Resources */,
...
\t\t\tdependencies = (
\t\t\t);
\t\t\tname = SlopeTrackWatch;
\t\t\tproductName = SlopeTrackWatch;
\t\t\tproductReference = %s /* SlopeTrackWatch.app */;
...
\t\t\t\tCODE_SIGN_ENTITLEMENTS = SlopeTrackWatch/SlopeTrackWatch.entitlements;
\t\t\t\tCODE_SIGN_STYLE = Automatic;
\t\t\t\tCURRENT_PROJECT_VERSION = "$(FLUTTER_BUILD_NUMBER)";
\t\t\t\tDEVELOPMENT_TEAM = 5GDU97KSQU;
\t\t\t\tENABLE_PREVIEWS = YES;
\t\t\t\tGENERATE_INFOPLIST_FILE = NO;
\t\t\t\tINFOPLIST_FILE = SlopeTrackWatch/Info.plist;
\t\t\t\tLD_RUNPATH_SEARCH_PATHS = (
\t\t\t\t\t"$(inherited)",
\t\t\t\t\t"@executable_path/Frameworks",
\t\t\t\t);
\t\t\t\tMARKETING_VERSION = "$(FLUTTER_BUILD_NAME)";
\t\t\t\tPRODUCT_BUNDLE_IDENTIFIER = de.torchtechnology.slopetrack.watchkitapp;
...
''' % (cid, name, settings, extra, name)
    ...
    s = s.replace('/* End XCConfigurationList section */', '''\t\t%s /* Build configuration list for PBXNativeTarget "SlopeTrackWatch" */ = {
...
/* End XCConfigurationList section */''' % (i('04'), i('05'), i('07'), i('06')), 1)
    ...
    open(PATH, 'w').write(s)
    print('SlopeTrackWatch target added to', PATH)
    return 0
