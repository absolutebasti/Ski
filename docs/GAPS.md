# Gaps — what is still missing (audit 2026-09-24, updated the same evening)
...
| 5 | ~~Wochen-Challenge card is empty~~ **Closed 2026-09-24:** migration 0003 seeds three challenges per ISO week and pg_cron creates them every Monday: `challenges` table has 0 rows and nothing creates weekly challenges | The third competition feature shows nothing | backend | S | supabase/migrations, features/social/challenge_card.dart |
| 6 | ~~Season goal shown nowhere~~ **Closed 2026-09-24:** season card shows 'ZIEL … · %' with a progress rule (only `settings.dart` and `onboarding_flow.dart` reference it) | The onboarding promise ("your goal") has no follow-through on Heute/Tage | code | S | features/today/season_card.dart |
| 7 | ~~Simulator shows the location-denied card~~ **Closed 2026-09-24:** demo mode overrides the permission service on Heute; the permission grant via `simctl privacy` does not reach the app | Demo/screenshot flow shows a warning card instead of the last day | code | S | tools/shots.sh |
...
| 15 | ~~iPad device family~~ **Closed 2026-09-24:** iPhone only while the store plan says iPhone only | Either iPad screenshots + layouts, or set to iPhone only | code | S | ios/Runner.xcodeproj |
...
| 19 | ~~Leftover names~~ **Closed 2026-09-24** with the SlopeTrack rename (migration filename kept) in docs/PLAN.md, migration filename, Watch files (`SchwungWatchApp.swift`, `SchwungWatch.entitlements`), two widget comments | Cosmetic, but confusing for review and for the Watch target script | code | S | grep -ri schwung |
| 20 | ~~MyRank.total is the slice size~~ **Closed 2026-09-24:** leaderboard RPC returns a window count (limit 100), not a server-side participant count | "Platz 14 von 100" is wrong for big resorts | backend | S | features/social/social_models.dart |
